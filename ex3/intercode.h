...
{
	enum{
		VARIABLE_OP,TEMP_VAR_OP,CONSTANT_OP,ADDRESS_OP,TEMP_ADDR_OP,LABEL_OP,FUNCTION_OP,DEBUG_OP
	} kind;
	union {
...
typedef struct InterCode_t
{
	...
	union {
		//1操作数: LABEL_IR FUNCTION_IR GOTO_IR RETURN_IR ARG_IR PARAM_IR READ_IR WRITE_IR DEBUG_IR
		struct
		{
			Operand op;
		} singleOP;

		//2操作数: ASSIGN_IR,GET_VALUE_IR,TO_MEMORY_IR,CALL_IR
		struct
		{
			Operand left;
			Operand right;
		...
		//3操作数: PLUS_IR,MINUS_IR,STAR_IR,DIV_IR,GET_ADDR_IR
		struct
		{
			Operand result;
			Operand op1;
